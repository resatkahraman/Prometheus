...
const SIDECAR_NAME: &str = "prometheus-core-x86_64-pc-windows-msvc.exe";
...
pub async fn start(app: AppHandle, shared: SharedRuntime) -> Result<RuntimeStatus, String> {
    ...
    let (executable, args, working_dir) = if cfg!(debug_assertions) { let executable = match python_path() { Some(path) => path, None => { let mut guard = shared.lock().map_err(|_| "runtime_lock".to_string())?; set_error(&mut guard, RuntimeLifecycle::LaunchUnavailable, "launch_unavailable", "Canonical development Core launcher is unavailable."); return Err("launch_unavailable".into()); } }; let root = match repo_root() { Some(path) => path, None => { let mut guard = shared.lock().map_err(|_| "runtime_lock".to_string())?; set_error(&mut guard, RuntimeLifecycle::LaunchUnavailable, "launch_unavailable", "Canonical development Core workspace is unavailable."); return Err("launch_unavailable".into()); } }; (executable, vec!["-m", "app.desktop_server"], root) } else { let executable = match bundled_core_path(&app) { Ok(path) => path, Err(code) => { let mut guard = shared.lock().map_err(|_| "runtime_lock".to_string())?; set_error(&mut guard, RuntimeLifecycle::LaunchUnavailable, "core_runtime_not_found", "The packaged Prometheus Core runtime is unavailable."); return Err(code); } }; let working_dir = match app.path().app_data_dir() { Ok(path) => path, Err(_) => { let mut guard = shared.lock().map_err(|_| "runtime_lock".to_string())?; set_error(&mut guard, RuntimeLifecycle::LaunchUnavailable, "core_runtime_not_found", "The writable Core data directory is unavailable."); return Err("core_runtime_not_found".into()); } }; if std::fs::create_dir_all(&working_dir).is_err() { let mut guard = shared.lock().map_err(|_| "runtime_lock".to_string())?; set_error(&mut guard, RuntimeLifecycle::LaunchUnavailable, "core_runtime_not_found", "The writable Core data directory is unavailable."); return Err("core_runtime_not_found".into()); } (executable, Vec::new(), working_dir) };
    let mut command = Command::new(executable); command.current_dir(working_dir).args(args); configure_child_process(&mut command); let child = match command.spawn() { Ok(child) => child, Err(_) => { let mut guard = shared.lock().map_err(|_| "runtime_lock".to_string())?; set_error(&mut guard, RuntimeLifecycle::LaunchFailed, "core_start_failed", "The Prometheus Core runtime could not be started."); return Err("core_start_failed".into()); } };
