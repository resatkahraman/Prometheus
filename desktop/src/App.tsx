...
import { approveDesktopTask, getDesktopBootstrap, getDesktopCoreStatus, getDesktopMission, getDesktopMissionEvents, rejectDesktopTask, submitDesktopCommand } from './lib/desktopBridge';
import type { CoreStatus, DesktopCommandResponse, Mission, MissionEvent, NavigationView } from './types/desktop';
...
export default function App() {
  ...
  const missionGeneration = useRef(0);
  ...
  useEffect(() => { if (!missionId) { setMission(null); setMissionEvents([]); return; } const generation = ++missionGeneration.current; let disposed = false; let inFlight = false; const refresh = async () => { if (inFlight) return; inFlight = true; try { const [nextMission, nextEvents] = await Promise.all([getDesktopMission(missionId), getDesktopMissionEvents(missionId)]); if (!disposed && missionGeneration.current === generation) { setMission(nextMission); setMissionEvents(nextEvents.events); } } catch (error) { if (!disposed && missionGeneration.current === generation) setDecisionMessage(typeof error === 'object' && error && 'code' in error ? String(error.code) : t.missionLoadFailed); } finally { inFlight = false; } }; void refresh(); const timer = window.setInterval(() => { if (!mission?.status || !['completed', 'failed', 'cancelled'].includes(mission.status)) void refresh(); }, 2000); const onFocus = () => { void refresh(); }; window.addEventListener('focus', onFocus); return () => { disposed = true; window.clearInterval(timer); window.removeEventListener('focus', onFocus); }; }, [missionId, mission?.status, t.missionLoadFailed]);
